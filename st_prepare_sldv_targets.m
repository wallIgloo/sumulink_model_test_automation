...
function validate_harness_input_interface( ...
        harnessInput, ...
        datasetNames, ...
        datasetTypes, ...
        datasetDimensions)
%VALIDATE_HARNESS_INPUT_INTERFACE
% Compare an SLDV Dataset against the actual Harness ActiveScenario input
% interface. Order is significant because Signal Editor Dataset order is the
% runtime input order used by the Harness.

harnessNames = ...
    harnessInput.Names;

if numel(harnessNames) ~= numel(datasetNames) || ...
        ~isequal( ...
            string(harnessNames(:)), ...
            string(datasetNames(:)))

    error( ...
        ['SLDV Dataset/Harness ActiveScenario input interface mismatch. ' ...
         'Scenario=%s, Harness=[%s], SLDV=[%s]'], ...
        harnessInput.ScenarioName, ...
        strjoin(harnessNames, ', '), ...
        strjoin(datasetNames, ', '));
end

for i = 1:numel(harnessNames)

    if ~strcmpi( ...
            harnessInput.Types{i}, ...
            datasetTypes{i})
